"""
Smart RAG 시스템 (챗봇 버전) - 전체 기능 포함 최종본
...
def normalize_korean_numbers(text: str) -> str:
    ...
    patterns: list[tuple[str, Callable[[re.Match], str]]] = [
        ...
        (r"(\d+)\.(\d+)\s*만", lambda m: str(int(float(f"{m.group(1)}.{m.group(2)}") * 10000))),
    ]
...
DISASTER_KEYWORDS: dict[str, str] = {
    ...
    "태풍": "태풍", "typhoon": "태풍",
    "폭풍": "폭풍", "storm": "폭풍",
    "홍수": "홍수", "침수": "홍수", "flood": "홍수",
    "산사태": "산사태", "landslide": "산사태",
    "해일": "해일", "쓰나미": "쓰나미", "tsunami": "쓰나미",
    "화산": "화산", "volcanic": "화산",
    "가스": "가스", "gas": "가스",
    "산불": "산불", "wildfire": "산불",
    "방사능": "방사능", "방사선": "방사능", "radiation": "방사능",
    "댐": "댐", "dam": "댐",
}
...
답변: AGGREGATION, FILTER, SHELTER_INFO, GUIDELINE, GENERAL 중 하나"""),
    ...
    def extract_conditions(self, query: str) -> QueryAnalysis:
        ...
        prompt = ChatPromptTemplate.from_messages([
            ("system", """질문에서 대피소 검색 조건을 추출하여 JSON으로 반환하라.
...
class ResultFormatter:
    ...
    def format_aggregation(result: FilterResult) -> str:
        ...
        response = f"총 {result.count}곳입니다."
    ...
    def format_filter(
        ...
        response = f"총 {result.count}곳이 검색되었습니다.\n"
...
5. 행동요령은 단계별로 명확하게 안내한다."""
    ...
    def invoke(self, query: str) -> str:
        ...
        if not self.all_documents:
            return "DB에 문서가 없습니다. from_DataLoad_to_VectorDB.py를 먼저 실행하세요."
    ...
    def search(
        ...
        Args:
            query: 검색 쿼리
            k: 반환할 문서 수
            source_filter: 소스 타입 필터 (shelter/guideline)
    ...
    def _handle_filter(self, query: str) -> str:
        """필터 쿼리 처리."""
        analysis = self.query_analyzer.extract_conditions(query)
    ...
    def _handle_shelter_info(self, query: str) -> str:
        """특정 대피소 정보 조회."""
        docs = self.hybrid_retriever.invoke(query, SourceType.SHELTER)
        return self.formatter.format_shelter_info(docs) if docs else "해당 시설 정보를 찾을 수 없습니다."

    def _handle_guideline(self, query: str) -> str:
        """재난 행동요령 질문 처리."""
...
# =============================================================================
# 실행 (Main) - 챗봇 스타일 대화형 인터페이스
# =============================================================================


def format_stats(stats: DBStats) -> str:
    """통계를 문자열로 포맷."""
    lines = [
        f"총 문서: {stats.total_documents}개",
        f"대피소: {stats.shelter_count}개",
        f"행동요령: {stats.guideline_count}개",
    ]

    if stats.shelter_stats:
        ss = stats.shelter_stats
        lines.append(f"\n[대피소 통계]")
        lines.append(f"수용인원: 최소 {ss.capacity_min:,} / 최대 {ss.capacity_max:,} / 평균 {ss.capacity_avg:,.0f}")
        lines.append(f"총 수용가능: {ss.capacity_total:,}명")
        lines.append(f"위치유형: {ss.shelter_types}")
        lines.append(f"상위 지역: {ss.top_districts}")

    if stats.guideline_stats:
        gs = stats.guideline_stats
        lines.append(f"\n[행동요령 통계]")
        lines.append(f"재난유형: {gs.disaster_types}")
...
def main() -> None:
    ...
    while True:
        try:
            ...
            # 종료 명령
            if user_input.lower() in ["quit", "exit", "종료", "끝", "/quit", "/q"]:
                print("\nBot: 안녕히 가세요! 안전한 하루 되세요!\n")
                break
            ...
            if user_input.lower() in ["/stats", "/s"]:
                stats = rag.get_stats()
                print(f"\nBot: {format_stats(stats)}\n")
                continue

            # 예시
            if user_input.lower() in ["/examples", "/e"]:
                print(f"\nBot: {print_examples()}\n")
                continue
            ...
            print(f"\nBot: 죄송합니다, 오류가 발생했습니다. 다시 질문해주세요.\n")
